...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR arg, int) {
    ...
    std::string directoryPath = fullArg.substr(6); // everything after first 5 chars and the whitespace
    ...
    */
    ...
    if (RegOpenKeyExA(HKEY_CURRENT_USER, regKeyPath.c_str(), 0, KEY_READ | KEY_WRITE, &hSubKey) == ERROR_SUCCESS) {
        ...
        while (true) {
            ...
            if (RegEnumValueA(hSubKey, 0, valueName, &nameLen, NULL, NULL, NULL, NULL) != ERROR_SUCCESS)
                break;
            RegDeleteValueA(hSubKey, valueName);
        }
